...
export default function ProfileScreen() {
  const { t, i18n } = useTranslation(); const { colors, spacing, radius, type, isDark } = useTheme(); const router = useRouter(); const { user, loading } = useProfile(); const { isPremium } = usePremium(); const [notifications, setNotifications] = React.useState(true)
  React.useEffect(() => { if (!loading && !user) router.replace("/auth/login") }, [loading, user, router])
  if (!user) return <Screen header={<><Stack.Screen options={{ headerShown: false }} /><AppHeader title="Profile" /></>} />
  return <Screen scroll header={<><Stack.Screen options={{ headerShown: false }} /><AppHeader title="Profile" /></>}>
    <View style={[styles.account, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md }]}><Text style={[type.overline, { color: colors.accent }]}>ACCOUNT</Text><Text style={[type.title, { color: colors.text, marginTop: 10 }]}>{user.full_name || "Your account"}</Text><Text style={[type.body, { color: colors.textMuted, marginTop: 4 }]}>{user.email || user.phone_number}</Text>{isPremium && <Text style={[type.overline, { color: colors.success, marginTop: 12 }]}>MEMBERSHIP ACTIVE</Text>}</View>
    ...
    <Text style={[type.overline, { color: colors.accent, marginTop: spacing.xl }]}>PREFERENCES</Text><View style={[styles.group, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, marginTop: spacing.sm }]}><Row icon="language-outline" label="Language" value={i18n.language === "en" ? "English" : "Swahili"} colors={colors} onPress={() => setLanguage(i18n.language === "en" ? "sw" : "en")} /><Row icon="notifications-outline" label="Notifications" colors={colors} right={<Switch value={notifications} onValueChange={setNotifications} trackColor={{ true: colors.success, false: colors.border }} />} /><Row icon="moon-outline" label="Appearance" value={isDark ? "Dark" : "Light"} colors={colors} /></View>
    <Text style={[type.overline, { color: colors.accent, marginTop: spacing.xl }]}>ABOUT</Text><View style={[styles.group, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.md, marginTop: spacing.sm }]}><Row icon="card-outline" label="Subscription" colors={colors} onPress={() => router.push("/subscription")} /><Row icon="information-circle-outline" label="About" value="Version 1.0.0" colors={colors} /><Row icon="log-out-outline" label="Log out" colors={colors} onPress={async () => { await logout(); Alert.alert("Log out", "You have been signed out.") }} /></View>
  </Screen>
}
