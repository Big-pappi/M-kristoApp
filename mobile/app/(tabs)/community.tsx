...
export default function CommunityScreen() {
  ...
  return <Screen header={<AppHeader title={isEnglish ? "Community" : "Jamii"} />}>
    ...
    <View style={{ gap: spacing.md, marginTop: spacing.xl }}>{posts.map((post) => <View key={post.id} style={[styles.post, { backgroundColor: colors.surface, borderColor: colors.border, borderRadius: radius.lg }]}>
      <View style={styles.postHead}><View style={[styles.avatar, { backgroundColor: colors.accentSoft }]}><Text style={[type.heading, { color: colors.accent }]}>{post.name.slice(0, 1)}</Text></View><View style={{ flex: 1 }}><Text style={[type.heading, { color: colors.text }]}>{post.name}</Text><Text style={[type.caption, { color: colors.textMuted }]}>{isEnglish ? "Reading with the community" : "Anasoma na jamii"}</Text></View><Ionicons name="ellipsis-horizontal" size={18} color={colors.textMuted} /></View>
