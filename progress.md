...
## 2026-08-20 — Multi-version Bible support with audio (COMPLETED)

**Status:** Full implementation of multi-version Bible support with audio playback functionality.

**Done:**
### Database Schema Changes
- Updated `architecture/database_schema.sql` to support multiple Bible versions:
  - Added `bible_versions` table with codes for NIV, KJV, NLT, LSB, ESV, CPDV
  - Updated `bible_verses` table to include `version_id` foreign key
  - Added `audio_url_sw` and `audio_url_en` columns for audio translations
  - Added unique constraint on `(book_id, version_id, chapter, verse_number)`
  - Inserted sample Bible version data for both English and Swahili versions
- Updated `architecture/DATABASE_SCHEMA.md` to reflect new schema structure
- Updated entity relationships to include `bible_versions` 1—N `bible_verses`

### Mobile App File Structure
- Reorganized `mobile/books/` directory structure:
  - Created `versions/` subdirectory with folders for each Bible version (NIV, KJV, NLT, LSB, ESV, CPDV)
  - Created `audio/` subdirectory with language-specific folders (en/sw) for each version
  - Moved existing KJV JSON files to `versions/KJV/` directory
  - Added comprehensive `README.md` documenting the new structure
  - Created sample NIV Genesis.json file with audio URL structure

### API and Type Updates
- Updated `mobile/src/api/bible.ts`:
  - Added `BibleVersion` type with code, names (sw/en), language, and active status
  ...
  - Updated `listVerses()` and `searchVerses()` to accept optional `versionId` parameter
  - Updated `getLocalBookData()` to accept version parameter
  - Added `getLocalBibleVersions()` function returning available versions
  - Updated import paths to use new `versions/KJV/` directory structure

### New React Hook
- Created `mobile/src/hooks/useBibleVersion.ts`:
  - Manages Bible version selection state with AsyncStorage persistence
  - Provides version list, current selection, and setter function
  - Integrates with AsyncStorage for user preference persistence

### UI Updates
- Updated `mobile/app/(tabs)/bible.tsx`:
  - Added version selector UI component
  - Integrated `useBibleVersion` hook for version management
  - Added expandable version list with selection indicators
  - Updated book navigation to pass selected version parameter
  - Added bilingual version name display

- Updated `mobile/app/bible/[bookId].tsx`:
  - Added audio playback functionality using expo-av
  - Integrated `useBibleVersion` hook for version-aware data loading
  - Added audio control icons (play/pause) for verses with audio
  - Implemented audio state management (current audio, playing state)
  - Added audio cleanup on component unmount
  - Updated verse data loading to use version parameter
  - Enhanced verse actions row to include audio controls

### Dependencies
- Updated `mobile/package.json`:
  - Added `expo-av: ~16.0.11` for audio playback functionality

**Next up:**
- Install new dependencies (`npm install` in mobile directory)
- Add actual Bible data files for additional versions (NIV, NLT, LSB, ESV, CPDV)
- Add audio files to the `audio/` directory structure
- Update backend Django models to match new schema
- Implement backend API endpoints for version management
- Test audio playback functionality on different devices

**Notes:**
- Database now supports 6 Bible versions (NIV, KJV, NLT, LSB, ESV, CPDV) in both English and Swahili
- Each verse can have separate audio files for both languages
- Mobile app now has full version selection UI and audio playback capabilities
- User's version preference is persisted using AsyncStorage
- Audio files should follow naming convention: `{VERSION}/{LANGUAGE}/{BOOK}/{BOOK}_{CHAPTER}_{VERSE}.mp3`
- Current implementation uses KJV as default version with existing data
- Backend API will need corresponding updates to handle version-specific queries
